...
train = pd.read_csv("fashion-mnist_train.csv", index_col=0)
x_train = train.to_numpy(np.uint8)
x_train = x_train.reshape(x_train.shape[0], 28, 28)
y_train = np.array(train.index.values)
test = pd.read_csv("fashion-mnist_test.csv", index_col=0)
x_test = test.to_numpy(np.uint8)
x_test = x_test.reshape(x_test.shape[0], 28, 28)
y_test = np.array(test.index.values)
...
def conv_forward(x, w, b):
    ...
    padding_h = (FH - 1) // 2  # padding on height
    padding_w = (FW - 1) // 2  # padding on width
    ...
    for i in range(N):
        ...
        for j in range(0, padded_H - FH + 1, stride):
            for k in range(0, padded_W - FW + 1, stride):
                x_col[:, c] = padded_x[i, :, j : j + FH, k : k + FW].reshape(
                    C * FH * FW
                )
                c += 1
        out[i, :] = (np.dot(w_row, x_col) + b.reshape(-1, 1)).reshape(F, H, W)

    cache = (x, w, b, stride, padding_h, padding_w)
    return out, cache
...
def conv_backward(dout, cache):
    ...
    """
    x, w, b, stride, padding_h, padding_w = cache
    N, C, H, W = x.shape
    F, _, FH, FW = w.shape
    ...
    for i in range(N):
        ...
        for j in range(0, padded_H - FH + 1, stride):
            for k in range(0, padded_W - FW + 1, stride):
                ...
                c += 1
        dx[i] = curr_dpx[:, padding_h:-padding_h, padding_w:-padding_w]
    ...
    for i in range(N):
        ...
        for j in range(0, H - pool_height + 1, stride):
            for k in range(0, W - pool_width + 1, stride):
                curr_region = x[i, :, j : j + pool_height, k : k + pool_width].reshape(
    ...
    for i in range(N):
        ...
        for j in range(0, H - pool_height + 1, stride):
            for k in range(0, W - pool_width + 1, stride):
                ...
                dx[i, :, j : j + pool_height, k : k + pool_height] = (
                    curr_dpooling.reshape(C, pool_height, pool_width)
                )
                c += 1
...
def softmax_loss(x, y):
    ...
    numerator = np.exp(x)
    probs: np.ndarray = numerator / np.sum(numerator, axis=1, keepdims=True)
...
class CustomNet(object):
    def __init__(
        ...
        C, H, W = input_dim
        self.W1 = np.random.normal(0.0, weight_scale, (hidden_dim, C, 3, 3))
        ...
        self.W2 = np.random.randn(
            conv_out_H * conv_out_W * hidden_dim, num_classes
    ...
    def forward(self, x):
        # forward pass
        x, conv_cache = conv_forward(x, self.W1, self.b1)
        x, pool_cache = max_pool_forward(x)
        x, fc_cache = fc_forward(x, self.W2, self.b2)
    ...
    def loss(self, x, y):
        ...
        print("Backdrop...")
        # backprop
        dout, dW2, db2 = fc_backward(dout, fc_cache)
        dout = max_pool_backward(dout, pool_cache)
        dout, dW1, db1 = conv_backward(dout, conv_cache)
    ...
    def train(self, x, y, lr=1e-4, batch_size=128, epochs=10):
        ...
        print(
            "Splitted the training set into {} mini batches. \n".format(
                len(mini_batches)
            )
        )
...
class ConvNet(object):
    def __init__(
        ...
        self.W3 = np.random.randn(64 * conv_out_H * conv_out_W, hidden_dim) * np.sqrt(
            2.0 / (64 * conv_out_H * conv_out_W)
        )
...
model = CustomNet()
losses = model.train(x_train, y_train, batch_size=1000, epochs=1)
